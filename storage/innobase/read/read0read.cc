...
MVCC::get_view() {
    ...
    // m_free 可能是一个缓存的意思
    // 从缓存中看看有没有可以用的 view 对象，如果有，取出来然后从 m_free 中删除
...
MVCC::view_open(ReadView *&view, trx_t *trx) {
    ...
    if (view != NULL) {
        ...
        // 创建 ReadView 要加锁，因为需要把系统中当前被激活的所有事务的 no 复制到 ReadView 中
        // 锁是谁的锁呢？一个所谓的事务系统的锁
        mutex_enter(&trx_sys->mutex);

        // 会从空闲 ReadView 链表中获取一个空间的 ReadView 对象，没有空闲的就会构建一个空的 ReadView 对象
        view = get_view();
    }
    ...
    // 上面已经获取到 ReadView
    if (view != NULL) {
        // 初始化 m_creator_trx_id、m_low_limit_id、m_low_limit_no、m_ids
        // m_low_limit_id表示下一个事务的id

        // 传入当前事务 id，里面会赋值给 creator_trx_id
        view->prepare(trx->id);
