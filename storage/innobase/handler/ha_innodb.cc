...
create_table_info_t::create_table() {
    ...
    // 其实就是检查主键的索引是多少，一般第一个就是主键，索引是 0
    primary_key_no = (m_form->s->primary_key != MAX_KEY ? (int) m_form->s->primary_key : -1);

    /* Our function innobase_get_mysql_key_number_for_index assumes
    the primary key is always number 0, if it exists */
    // 断言要不就是找不到主键，要不就是主键在第 0 个位置
...
/*===================*/
{
    ...
    switch (err) {
        ...
        case DB_TABLESPACE_NOT_FOUND:
            ib_senderrf(
                m_prebuilt->trx->mysql_thd, IB_LOG_LEVEL_ERROR,
                (err == DB_TABLESPACE_DELETED ? ER_TABLESPACE_DISCARDED : ER_TABLESPACE_MISSING),
                table->s->table_name.str);
            table->status = STATUS_NOT_FOUND;
            error = HA_ERR_TABLESPACE_MISSING;
            break;
    ...
    if (*(my_bool *) save) {
        while (log_sys->last_checkpoint_lsn
               ...
               + (log_sys->append_on_checkpoint != NULL
                      ? log_sys->append_on_checkpoint->size()
                      : 0)
               < log_sys->lsn) {
